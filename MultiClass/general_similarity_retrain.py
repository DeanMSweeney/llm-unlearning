...
import json
import sys
from dataset import WGDataset, HolisticGroupDataset
...
def update_model_param_grads(
    ...
    grads_2,
    device,
    new_grad_calc=_minimize_grads_2,
):
    """
    By convention, grads_1 should be the "pos grads" and grads_2 should be the "neg grads".
    For new_grad_calc, remember that optimizer steps in the direction of minimization
    (i.e., opposite direction of what new_grad_calc returns).
    """

    # This will set .grad to None with modern PyTorch (set_to_none=True by default)
    optimizer.zero_grad()
    ...
    if params_to_keep is not None:
        for param_name, indices in params_to_keep:
            ...
            if indices is None:
                ...
                new_grad = new_grad_calc(
                    grads_1[param_name][indices],
                    grads_2[param_name][indices],
                )
...
def retrain_multiclass(model, tokenizer, optimizer, device, dataloader: data.DataLoader, group_batch_size: int,
                        k: int = 10000, num_epochs: int = 5,
                        sim_batch_size: int = -1, new_grad_calc=_minimize_grads_2,
                        agg_dim: int = -1, start_at_epoch: int = 0,
                        dedupe: str = '', model_name: str = ''):

    logger.info('retraining mlm (multi-class HolisticBias setting)')

    if sim_batch_size == -1:
        sim_batch_size = group_batch_size

    if sim_batch_size is not None and (sim_batch_size <= 0 or sim_batch_size % group_batch_size != 0):
        raise ValueError(f'Batch size for computing similarity is invalid: {sim_batch_size}')
    ...
    for epoch in range(start_at_epoch, num_epochs):
        logger.info(f'[Multiclass] On epoch {epoch + 1}/{num_epochs}')
        model.train()
        ...
        for padded_seqs, attention_masks, inds, target_tokens, B, M in tqdm(dataloader):
            # B: number of groups, M: number of classes per group
            optimizer.zero_grad()

            padded_seqs = padded_seqs.to(device)         # (B*M, T)
            attention_masks = attention_masks.to(device) # (B*M, T)
            inds = inds.to(device)                       # (B,)
            target_tokens = target_tokens.to(device)     # (B*M,)

            logits_per_class = []
            grads_per_class = []

            # Process each class separately to get per-class gradients
            for m in range(M):
                # Flattening order: [g0_c0, g0_c1, ..., g0_cM-1, g1_c0, g1_c1, ...]
                idxs = torch.arange(m, B * M, M, device=device)  # (B,)
                seqs_m = padded_seqs[idxs]                       # (B, T)
                attn_m = attention_masks[idxs]                   # (B, T)
                targets_m = target_tokens[idxs]                  # (B,)

                _, logits_m = do_an_mlm_backprop(
                    model,
                    seqs_m,
                    attn_m,
                    inds,
                ...
                grads_m = get_all_model_grads(model)

                logits_per_class.append(logits_m.detach().cpu())
                grads_per_class.append(grads_m)

                optimizer.zero_grad()

            # logits_per_class: list length M, each (B, 1)
            logits_mb = torch.stack([l.squeeze(-1) for l in logits_per_class], dim=0)  # (M, B)
            mean_logits = logits_mb.mean(dim=1)  # (M,)

            adv_idx = torch.argmax(mean_logits).item()

            # Build advantaged / disadvantaged gradients
            adv_grad = grads_per_class[adv_idx]
            disadv_grad = None
            count = 0
            for m, g_m in enumerate(grads_per_class):
                if m == adv_idx:
                    continue
                disadv_grad = accumulate_grad(disadv_grad, g_m)
                count += 1

            # Average disadvantaged grads over all non-advantaged classes
            for name in disadv_grad:
                if disadv_grad[name] is not None:
                    disadv_grad[name] /= count

            curr_disadv_grads = accumulate_grad(curr_disadv_grads, disadv_grad)
            curr_adv_grads = accumulate_grad(curr_adv_grads, adv_grad)

            curr_sim_batch_count += B
            ...
            if sim_batch_size is not None and curr_sim_batch_count >= sim_batch_size:
                take_optim_step(
                    ...
                    device,
                    k=k,
                    new_grad_calc=new_grad_calc,
                )
                curr_sim_batch_count = 0
                curr_disadv_grads = None
                curr_adv_grads = None

        if sim_batch_size is None and curr_disadv_grads is not None:
            take_optim_step(
                optimizer,
                params_map,
                param_partition,
        ...
        save_model(model, tokenizer, epoch + 1, dedupe=dedupe)
...
def main(model_path_or_name: str, num_epochs: int = 5, is_mlm: bool = True, k: int = 10000,
            ...
            num_workers: int = 4, start_at_epoch: int = 0, dedupe: str = '',
            holistic_tsv: str | None = None):
    """
    If holistic_tsv is None:
        - Use WGDataset (binary, original PCGU)
    Else:
        - Use HolisticGroupDataset (multi-class PCGU)
    """
    ...
    # Keep original behavior: use AutoModelForPreTraining
    ...
    if holistic_tsv is None:
        # Original WG binary setting
        logger.info('Using WGDataset (binary PCGU)')
        dataset = WGDataset('../data/wg.tsv', '../data/wg_stats.tsv', tokenizer)
        dataloader = data.DataLoader(
            dataset,
            batch_size=batch_size,
            collate_fn=WGDataset.collate_batch_creator(tokenizer),
            num_workers=num_workers,
        )

        logger.info('Retraining now (binary WG)')
        retrain(
            model,
            tokenizer,
            optimizer,
        ...
        # New HolisticBias multi-class setting
        logger.info(f'Using HolisticGroupDataset from {holistic_tsv}')
        dataset = HolisticGroupDataset(holistic_tsv, tokenizer)
        dataloader = data.DataLoader(
            dataset,
            batch_size=batch_size,  # groups per batch
            collate_fn=HolisticGroupDataset.collate_batch_creator(tokenizer),
            # num_workers=num_workers,
            num_workers=0, 
        )

        logger.info('Retraining now (multi-class HolisticBias)')
        retrain_multiclass(
            model,
            tokenizer,
            optimizer,
...
if __name__=='__main__': 
    ...
    parser.add_argument('--holistic_tsv', type=str, default=None,
                        help='Path to HolisticBias multi-class TSV. If set, uses HolisticGroupDataset and multiclass PCGU.')
    ...
    main(
        args.model_path_or_name, num_epochs=args.num_epochs, is_mlm=is_mlm, k=args.k, 
        ...
        holistic_tsv=args.holistic_tsv
    )
