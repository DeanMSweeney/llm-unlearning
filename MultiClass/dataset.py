...
class HolisticGroupDataset(Dataset):
    def __init__(
        self,
        dataset_file: str,
        tokenizer,
        ...
        with open(dataset_file, encoding="utf-8") as f:
            ...
            for line in reader:
                ...
                if len(tok_phrases) == 0:
                    continue

                # Find first differing token index across phrases
                ref = tok_phrases[0]
                min_len = min(len(x) for x in tok_phrases)
                diff_idx = None
                for i in range(min_len):
                    tok_i = [x[i] for x in tok_phrases]
                    if not all(t == tok_i[0] for t in tok_i):
                        diff_idx = i
                        break
                if diff_idx is None:
                    continue

                # Collect target tokens at diff_idx
                target_tokens: List[int] = []
                ok = True
                for ts in tok_phrases:
                    if diff_idx >= len(ts):
                        ok = False
                        break
                    target_tokens.append(ts[diff_idx])
                if not ok:
                    continue

                # Replace that token with [MASK] in all phrases
                masked_sents: List[List[int]] = []
                for ts in tok_phrases:
                    ts = ts.copy()
                    ts[diff_idx] = mask_id
                    masked_sents.append(ts)

                self.examples.append((masked_sents, diff_idx, target_tokens))

        if len(self.examples) == 0:
            raise RuntimeError(
                f"No valid examples found in {dataset_file}. "
                f"Check that the TSV format matches prepare_holistic_groups.py."
            )
    ...
    def collate_batch_creator(tokenizer):
        ...
        Input batch: list of (masked_sents, diff_idx, target_tokens)
            masked_sents: list length M (classes), each is list[int]
            diff_idx: int
            target_tokens: list[int] length M

        Output:
            padded_seqs: (B*M, T)
            attention_masks: (B*M, T)
            inds: (B,)  mask index per group
            target_tokens: (B*M,)
            B: int, number of groups
            M: int, number of classes per group
        """
        pad_id = tokenizer.pad_token_id

        def collate_batch(batch):
            masked_sents_list, diff_indices, target_tokens_list = zip(*batch)
            B = len(masked_sents_list)
            M = len(masked_sents_list[0])

            flat_sents: List[List[int]] = [
                s for group in masked_sents_list for s in group
            ]
            flat_targets: List[int] = [
                t for group in target_tokens_list for t in group
            ]

            seq_tensors = [
                torch.tensor(s, dtype=torch.long) for s in flat_sents
            ]
            ...
            attention_masks = torch.ones_like(padded_seqs)
            attention_masks[padded_seqs == pad_id] = 0

            inds = torch.tensor(diff_indices, dtype=torch.long)  # (B,)
            target_tokens = torch.tensor(flat_targets, dtype=torch.long)  # (B*M,)

            return padded_seqs, attention_masks, inds, target_tokens, B, M
