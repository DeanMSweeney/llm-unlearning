...
class WGDataset(data.Dataset):
    ...
    def collate_batch_creator(tokenizer):
        """
        Creates collate function that keeps disadvantaged/advantaged sentences separate.
        Returns tuple pairs for sequences, masks, target tokens, and labels.
        This allows comparing model predictions for disadvantaged vs advantaged groups.
        """
        def collate_batch(batch):
            sent_pairs, inds, target_token_pairs = [list(unzipped) for unzipped in zip(*batch)]
            ...
            # Pad sequences separately for disadvantaged and advantaged groups
